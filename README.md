...
* **21.11.2024** Генетический алгоритм (PMX crossover, `768, 32, 128`): 63740.635
* **20.11.2024** Генетический алгоритм (те же параметры) после фикса DBL_MAX: 63736.547
* **20.11.2024** Генетический алгоритм (PMX crossover, `500, 20, 100`): 63325.121
* **20.11.2024** Переработан смысл файла util, теперь он block_dp, функции метрики вынесены в `cheker`
* **20.11.2024** #define MINGW добавляет дп по блокам. Просто Visual Studio не знает `__builtin_ctz`
* **20.11.2024** #define INPUT в includes означает файловый/консольный запуск, без него пойдёт прогон на сгенерированных тестах 
* **20.11.2024** Добавлен файл `includes.h`, в котором содержатся include необходимых стандартных библиотек + using's + #define's
* **20.11.2024** В файле solve.cpp появилась функция solve, которая работает консольно. Файл `tester` теперь предназначен для тестирования на своих тестах.
